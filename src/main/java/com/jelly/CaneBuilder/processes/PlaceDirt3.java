...
public class PlaceDirt3 extends ProcessModule{
    ...
    public void onTick() {
        if(onSecondLayer) {
            ...
            if ((int) mc.thePlayer.posZ == CaneBuilder.corner1z && blockStandingOn != Blocks.air) {
                CaneBuilder.switchToNextProcess(this);
            } else {
                AngleUtils.hardRotate(CaneBuilder.corner2z > CaneBuilder.corner1z ? 0 : 180);
            }
        }
    }
    @Override
    public void onEnable(){
        ...
        if(blockStandingOn.equals(Blocks.dirt) && blockBelowStandingOn.equals(Blocks.dirt))
            onSecondLayer = true;
        else {
            mc.thePlayer.inventory.currentItem = 1;
            new Thread(() -> {
                ...
                threadSleep(1000);
                AngleUtils.smoothRotateTo(CaneBuilder.corner2z > CaneBuilder.corner1z ? 0 : 180, 1.2f);
                threadSleep(500);
                onSecondLayer = true;
            }).start();
        }
